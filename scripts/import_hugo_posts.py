...
Usage:
    API_SECRET_KEY=<key> python scripts/import_hugo_posts.py \
...
def main():
    parser = argparse.ArgumentParser(
        description="Import all Hugo posts into the Flask blog API.")
    parser.add_argument("--url", required=True,
                        help="Base URL of the blog (e.g. https://example.com)")
    parser.add_argument("--content-dir", default="app/content",
                        help="Path to Hugo content directory")
    parser.add_argument("--dry-run", action="store_true",
                        help="Parse files but do not POST")
    ...
    for md_file in posts:
        ...
        if args.dry_run:
            print(
                f"DRY-RUN {md_file.name}: slug={payload['slug']!r} title={payload['title']!r}")
            continue

        resp = sign_and_post(api_url, secret, payload)
        status = "OK" if resp.ok else "FAIL"
        print(f"[{status}] {md_file.name}: HTTP {
